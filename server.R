function(input, output) {
  ...
  output$TotalDeathCountByGender<-renderPlotly({
    ggplotly(df %>% group_by(Year, Sex) %>% summarise(Total = sum(Deaths, na.rm = TRUE)) %>%
  ...
  output$TotalDeathCountByRaceEthnicity<-renderPlotly({
    ggplotly(df %>% group_by(Year, Race.Ethnicity) %>% summarise(Total = sum(Deaths, na.rm = TRUE)) %>%
  ...
  output$TotalDeathCountByLeadingCause<-renderPlotly({
    ggplotly(df %>% group_by(Year, Leading.Cause) %>% summarise(Total = sum(Deaths, na.rm = TRUE))%>%
  ...
  output$HispanicData<-renderPlotly({
    ggplotly(his_sub %>% group_by(Year, Leading.Cause, Sex) %>% summarise(Total = sum(Age.Adjusted.Death.Rate, na.rm = TRUE)) %>% 
  ...
  output$AsianandPacificIslandersData<-renderPlotly({
    ggplotly(a_sub %>% group_by(Year, Leading.Cause, Sex) %>% summarise(Total = sum(Age.Adjusted.Death.Rate, na.rm = TRUE)) %>% 
      top_n(n = 7, wt = Total) %>% ggplot(aes(x=Year, y=Leading.Cause, size=Total, color=Sex)) +
  ...
  output$WhiteNonHispanicData<-renderPlotly({
    ggplotly(w_sub %>% group_by(Year, Leading.Cause, Sex) %>% summarise(Total = sum(Age.Adjusted.Death.Rate, na.rm = TRUE)) %>% 
      top_n(n = 7, wt = Total) %>% ggplot(aes(x=Year, y=Leading.Cause, size=Total, color=Sex)) +
  ...
  output$BlackNonHispanicData<-renderPlotly({
    ggplotly(b_sub %>% group_by(Year, Leading.Cause, Sex) %>% summarise(Total = sum(Age.Adjusted.Death.Rate, na.rm = TRUE)) %>% 
      top_n(n = 7, wt = Total) %>% ggplot(aes(x=Year, y=Leading.Cause, size=Total, color=Sex)) +
      geom_point(alpha=0.5) + scale_size(range = c(.1, 24), name="Black NonHispanic Leading Cause"))
  })
  
  output$OthersData<-renderPlotly({
    ggplotly(o_sub %>% group_by(Year, Leading.Cause, Sex) %>% summarise(Total = sum(Age.Adjusted.Death.Rate, na.rm = TRUE)) %>% 
      top_n(n = 7, wt = Total) %>% ggplot(aes(x=Year, y=Leading.Cause, size=Total, color=Sex)) +

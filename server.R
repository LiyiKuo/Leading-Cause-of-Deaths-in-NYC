function(input, output) {
  ...
  output$TotalDeathCountByGender<-renderPlot({
  df %>% group_by(Year, Sex) %>% summarise(Total = sum(Death.Rate, na.rm = TRUE)) %>%
      ggplot(aes(x=Year, y=Total, fill=Sex)) +
      geom_bar(position = "dodge", stat="identity") +
      labs(title="Total Death Count by Gender in NYC")
  })
  
  output$TotalDeathCountByRaceEthnicity<-renderPlot({
  df %>% group_by(Year, Race.Ethnicity) %>% summarise(Total = sum(Death.Rate, na.rm = TRUE)) %>%
      ggplot(aes(x=Year, y=Total, fill=Race.Ethnicity)) + 
      geom_bar(position = "dodge", stat="identity") + 
      labs(title="Total Death Count by Ethnicity in NYC")
  })
  
  output$TotalDeathCountByLeadingCause<-renderPlot({
  df %>% group_by(Year, Leading.Cause) %>% summarise(Total = sum(Death.Rate, na.rm = TRUE))%>%
  ...
  output$AgeAdjustedLeadingCauseOfDeath<-renderPlot({
  ...
  output$AgeAdjustedLeadingCauseOfDeathByRace<-renderPlot({
  sub %>% group_by(Race.Ethnicity, Leading.Cause) %>% summarise(Total = sum(Age.Adjusted.Death.Rate, na.rm = TRUE)) %>%
  ...
  output$AgeAdjHeartDiseaseData<-renderPlot({
  heart_sub %>% group_by(Year, Race.Ethnicity) %>% summarise(Total = sum(Age.Adjusted.Death.Rate, na.rm = TRUE))%>% 
  ...
  output$AgeAdjCancerData<-renderPlot({
  cancer_sub %>% group_by(Year, Race.Ethnicity) %>% summarise(Total = sum(Age.Adjusted.Death.Rate, na.rm = TRUE))%>% 
      top_n(n = 7, wt = Total) %>% ggplot(aes(x=Year, y=Total, fill=Race.Ethnicity)) + 
      geom_bar(position = "dodge", stat="identity") + labs(title="Total Death Count in Heart Disease by Ethnicity in NYC") + 
      coord_flip()
  })
